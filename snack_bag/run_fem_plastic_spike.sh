#!/usr/bin/env bash
# stage7 (E0509 + RH-P12 + GraspGen + cuRobo) 실행 런처
#  - dGPU(RTX 5080) 오프로드 렌더링
#  - 사용자 Wayland/Xwayland 세션 디스플레이 연결 (백그라운드 실행 시 창 표시)
#  - 종료: touch /tmp/fem_plastic_stop  (graceful, kill 불필요)
set -o pipefail
...
# 사용자 GUI 세션의 DISPLAY/XAUTHORITY 자동탐지 (재로그인 시 :0/:1·mutter/gdm 바뀜 → 하드코딩 금지)
...
cd ~/curobo_ws
exec python ~/shelf_grasp_dev/snack_bag/fem_plastic_spike.py "$@"
