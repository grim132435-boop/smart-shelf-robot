#!/usr/bin/env python3
# Stage6: RH-P12 그리퍼 정밀화(SDF 접촉·curl·오프셋) 작업 파일 — stage4에서 2026-06-12 분기
"""
Stage 6: E0509 + RH-P12-RN 그리퍼 오프셋·접촉 정밀화 (Isaac Sim GUI)

stage4_graspgen_e0509.py(동결)에서 분기. Stage5(추종 정밀화)는 stage5_graspgen_e0509.py(별도 세션).
분기 시점 포함분: Stage4 픽앤플레이스 전체 + plan_grasp(goalset) 파지선택 + joint4/5 비대칭 한계
  + Stage6 선행분(손가락 SDF collision=부족구동 curl 재현, 마찰 combine=max, r2/l2 curl 로깅).
이 파일에서 진행: Stage6 잔여 — 회귀검증, 캔 미세 기울기(TCP/축 오프셋) 보정.
실행: ./run_stage6.sh [--place --target-dy ...] / 종료: touch /tmp/stage4_stop 또는 pkill -9 -f stage6_graspgen
"""
...
#   닫기: touch /tmp/stage4_stop
STOP_FILE = "/tmp/stage4_stop"
...
def main():
    ...
    _gtest_done = False   # --gripper-test 1회 실행 플래그
    while simulation_app.is_running():
        ...
        if args.gripper_test and not _gtest_done:
            ...
            print("[GTEST] 완료 — logs/shots/shot_*_gtest_*.png를 실물 사진과 대조. "
                  "종료: touch /tmp/stage4_stop", flush=True)
            _gtest_done = True
            continue
        ...
        if state == GS.IDLE:
            ...
            if _cz_now[2] < _table_top + _half_q - 0.012:
                ...
                target_cube.set_world_pose(
                    position=np.array([_cx, _cy, _table_top + _half_q + 0.002]),
                    orientation=np.array([1.0, 0.0, 0.0, 0.0]))
                try:
                    target_cube.set_linear_velocity(np.zeros(3))
                    target_cube.set_angular_velocity(np.zeros(3))
            ...
            if _grasp_reached:
                ...
                try:
                    _tnames = [o.name for o in motion_gen.world_model.objects
                               if "target_cube" in o.name]
                    for _tn in _tnames:
                        motion_gen.world_coll_checker.enable_obstacle(enable=False, name=_tn)
                    print(f"  [충돌월드] 타겟 캔 장애물 해제({len(_tnames)}개)", flush=True)
                except Exception as _e:
            ...
            pre_center  = [SHELF3_X, SHELF3_PRE_Y, entry_z]
            ...
            _half_c = OBJ_SPECS["cylinder"]["height"] / 2.0
            entry_z = SHELF3_LIP_TOP + SHELF3_ENTRY_CLR + _half_c + grip_z_offset
            ins_from  = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_PRE_Y, entry_z], RHP12_TCP_DEPTH)
            ins_world = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_IN_Y,  entry_z], RHP12_TCP_DEPTH)
            print(f"  [P3] 매대 안 +y 진입(moveL) → y {SHELF3_PRE_Y}→{SHELF3_IN_Y} @TCPz {entry_z:.3f}", flush=True)
            ...
            low_from  = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_IN_Y, entry_z], RHP12_TCP_DEPTH)
            low_world = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_IN_Y, place_z], RHP12_TCP_DEPTH)
            ...
            _half_c = OBJ_SPECS["cylinder"]["height"] / 2.0
            place_z = SHELF3_FLOOR_TOP + SHELF3_REST_CLR + _half_c + grip_z_offset
            ret_from  = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_IN_Y,  place_z], RHP12_TCP_DEPTH)
            ret_world = side_grasp_from_approach(SHELF3_APPROACH, [SHELF3_X, SHELF3_PRE_Y, place_z], RHP12_TCP_DEPTH)
            print(f"  [P6] 매대 밖 -y 후퇴(moveL) → y {SHELF3_IN_Y}→{SHELF3_PRE_Y} @TCPz {place_z:.3f}", flush=True)
            ...
            if res_home.success.item():
                ...
                print("  [P8] ✅✅ 매대 3단(맨 위) 배치 + home 복귀 완료. 장면 유지(HALT).", flush=True)
            else:
