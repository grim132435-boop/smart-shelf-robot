...
def sample_object_pc(obj_type="box", n=NUM_PC_POINTS):
    ...
    if obj_type in ("cylinder", "bottle"):
        s = CYLSPEC
...
def select_best_reachable_grasp(grasps_world, scores, ik_solver, tensor_args, cu_js_seed,
    ...
    if grasp_mode == "side" and obj_center is not None:
        ...
        # 캔 파지 높이 = 중심 + GRASP_HEIGHT_FRAC·half. 0.7=상단부(손목 책상클리어↑, 3층 기본),
        #   0=중앙(그리퍼 최고점↓ → 2층 천장클리어). pick(책상)↔place(천장) 트레이드오프를 이 값으로 조절.
        grasp_center = np.array([obj_center[0], obj_center[1],
                                 obj_center[2] + GRASP_HEIGHT_FRAC * obj_half_h])
...
def main():
    ...
    if _obj_type in ("cylinder", "bottle") and args.objects > 1:
