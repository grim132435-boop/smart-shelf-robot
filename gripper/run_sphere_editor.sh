#!/usr/bin/env bash
# 충돌구체 저작용 Isaac Sim GUI 런처 (로봇만 로드, 물리 정지)
#  - run_stage4.sh와 동일한 dGPU 오프로드 + DISPLAY/XAUTHORITY 자동탐지
...
# 사용자 GUI 세션 DISPLAY/XAUTHORITY 자동탐지 (하드코딩 금지)
...
echo "[구체에디터런처] DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY (세션 자동탐지)"
cd ~/curobo_ws
exec python ~/shelf_grasp_dev/gripper/open_robot_for_spheres.py "$@"
